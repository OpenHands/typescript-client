...
# Start agent-server in Docker (software-agent-sdk v1.42.0)
docker run -d --name agent-server -p 127.0.0.1:8010:8000 \
  -v /tmp/agent-workspace:/workspace \
  ghcr.io/openhands/agent-server:1.42.0-python --host 0.0.0.0
...
- The integration workflow pins `ghcr.io/openhands/agent-server:1.42.0-python`, which corresponds to the `software-agent-sdk` release `v1.42.0`.
