...
class LocalConversationState {
    constructor(id) {
        ...
        this.confirmationPolicy = new NeverConfirm();
...
export class LocalConversation {
        ...
        if (this._state) {
            ...
            if ('requiresConfirmation' in policy) {
                ...
                this._state.confirmationPolicy = {
                    type: policy.type,
                    requiresConfirmation: () => policy.type === 'always',
                };
            }
        }
    }
     ...
     */
    async generateTitle(maxLength = 50, _llm) {
...
Provide a direct answer without using any tools.`;
        const messages = [...contextMessages, { role: 'user', content: askPrompt }];
     ...
     */
    async rejectPendingActions(reason = 'User rejected the action') {
        if (!this._state)
            return;
    ...
    emitEvent(event) {
        ...
        const typedEvent = {
            id: generateEventId(),
            kind: event.data?.kind || event.type,
...
//# sourceMappingURL=local-conversation.js.map