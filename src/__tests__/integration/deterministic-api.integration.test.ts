...
describe('Deterministic API Integration Tests', () => {
  const manager = new ConversationManager({ host: config.agentServerUrl, ...(config.apiKey ? { apiKey: config.apiKey } : {}) });
  const serverClient = new ServerClient({ host: config.agentServerUrl, ...(config.apiKey ? { apiKey: config.apiKey } : {}) });
  const llmClient = new LLMMetadataClient({ host: config.agentServerUrl, ...(config.apiKey ? { apiKey: config.apiKey } : {}) });
  const settingsClient = new SettingsClient({ host: config.agentServerUrl, ...(config.apiKey ? { apiKey: config.apiKey } : {}) });
  const skillsClient = new SkillsClient({ host: config.agentServerUrl, ...(config.apiKey ? { apiKey: config.apiKey } : {}) });
  const toolClient = new ToolClient({ host: config.agentServerUrl, ...(config.apiKey ? { apiKey: config.apiKey } : {}) });
  const vscodeClient = new VSCodeClient({ host: config.agentServerUrl, ...(config.apiKey ? { apiKey: config.apiKey } : {}) });
  const desktopClient = new DesktopClient({ host: config.agentServerUrl, ...(config.apiKey ? { apiKey: config.apiKey } : {}) });
  ...
  it('reads server, metadata, settings, tools, and skills endpoints', async () => {
  ...
  it('supports deterministic conversation, event, fork, and ACP endpoints', async () => {
    ...
    try {
      ...
      await workspace.executeCommand(`mkdir -p /workspace/conversations && printf 'trajectory-data' > ${trajectoryFile}`);
      ...
      const forkedConversation = await conversation.fork({ title: 'Forked from deterministic test' });
      forkedId = forkedConversation.id;
      expect(forkedConversation.id).not.toBe(conversation.id);

      await expect(conversation.switchProfile('__profile_that_should_not_exist__')).rejects.toBeInstanceOf(
        HttpError
      );
  ...
  it('uses preferred workspace file and git query endpoints', async () => {
